...
- [x] **24. Playback Visualization:** Extend the existing `SpotifyCard` with an action badge (Playing / Queued / Added to playlist), the target playlist name, and album art.
...
### Phase 6: Per-PR Preview Environments

Gives every pull request an isolated backend and frontend preview, wired to each other, so a feature can be reviewed end-to-end before it merges. Detailed design: `docs/design/phase_6_design.md`.

- [x] **25. Backend Health Endpoint & `render.yaml` Blueprint:** Add `GET /health` (status, commit, `isPreview`, service name) and commit a `render.yaml` defining `puding-backend` with `previews.generation: automatic`, `plan: free`, `healthCheckPath`, a server-scoped `buildFilter`, and a `puding-preview-secrets` environment group reference.
- [x] **26. Runtime Backend URL Resolution:** Replace the build-time `NEXT_PUBLIC_SERVER_WS_URL` read with a `?ws=` → `/api/config` → `NEXT_PUBLIC_SERVER_WS_URL` → localhost resolver, add the `/api/config` route handler deriving the per-PR backend from `VERCEL_ENV` and `VERCEL_GIT_PULL_REQUEST_ID`, and surface a "waking backend" state while a cold free-tier preview boots.
- [x] **27. Preview Pairing Workflow:** Add `.github/workflows/preview-env.yml` to resolve the real Render preview URL via the Render API, poll `/health`, push `PREVIEW_WS_URL_OVERRIDE` to the Vercel preview deployment, comment both URLs on the PR, and fail if the hostname convention no longer holds.
- [x] **28. Deployment Documentation & Verification:** Document the per-PR flow, the required Render/Vercel secrets and the environment group in `DEPLOYMENT.md`, with a reviewer checklist (open PR → both previews live → voice round-trip against the PR backend → teardown on close).

> Render free instances spin down after ~15 minutes idle, so the first connection to a cold preview takes ~30–50s, and every preview draws on the account's 750 free instance-hours per month.
