...
- [ ] **28. Deployment Documentation & Verification:** Document the per-PR flow, the required Render/Vercel secrets and the environment group in `DEPLOYMENT.md`, with a reviewer checklist (open PR → both previews live → voice round-trip against the PR backend → teardown on close).
