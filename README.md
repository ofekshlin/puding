...
- [ ] **27. Preview Pairing Workflow:** Add `.github/workflows/preview-env.yml` to resolve the real Render preview URL via the Render API, poll `/health`, push `PREVIEW_WS_URL_OVERRIDE` to the Vercel preview deployment, comment both URLs on the PR, and fail if the hostname convention no longer holds.
